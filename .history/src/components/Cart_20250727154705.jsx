...
const Cart = () => {
  ...
  const saveReceiptToFirestore = async (cartItems, total, receiptId) => {
    if (!receiptId || !cartItems || cartItems.length === 0) {
    ...
    const receiptData = {
      items: cartItems.map(item => ({
        productId: item.id || item.productId, // Ensures product ID is saved
    ...
    return (
      <div className='container mt-4'>
        ...
        <ReceiptImage
          ...
          receiptId={receiptId}
          onDone={async () => {
            await saveReceiptToFirestore(cartItems, total, receiptId)
