const ProductCard = ({ product, editMode, addStockMode, refreshProducts, role }) => {
  // ... same useState setup ...
  ...
  const handleAdd = () => { /* same as before */ }
      ...
      <div className='card h-100 shadow-sm'>
        ...
        <div className='card-body d-flex flex-column'>
          ...
          {editMode ? (
            <>
              ...
              <div className='mb-2'>
                ...
                <input
                  ...
                  onChange={e => setStoreQty(e.target.value === '' ? 0 : Math.max(0, parseInt(e.target.value)))}
                />
              </div>
              <div className='mb-2'>
                ...
                <input
                  ...
                  onChange={e => setShopQty(e.target.value === '' ? 0 : Math.max(0, parseInt(e.target.value)))}
                />
              </div>
              <div className='mb-2'>
              ...
              <p className='mb-1'><strong>Total:</strong> {totalQty}</p>
              <p className='mb-3'>
                <strong>Variance:</strong>
                <span className={`badge ${variance === 0 ? 'bg-success' : 'bg-danger'}`}>{variance}</span>
              </p>
              <button className='btn btn-primary mt-auto w-100' onClick={handleSave} disabled={isSaving}>
            ...
            <>
              <p className='mb-1'>
                <strong>Total:</strong> <span className='badge bg-secondary'>{totalQty}</span>
              </p>
              {!isCompanyUser && (
                <>
                  <p className='mb-1'><strong>Store:</strong> {storeQty}</p>
                  <p className='mb-1'><strong>Shop:</strong> {shopQty}</p>
                  {product.expiryDate && (
              ...
              <input
                ...
                min='1'
              />
              <button className='btn btn-outline-success w-100' onClick={handleAdd}>
                Add to Cart
              </button>
            </>
          )}
