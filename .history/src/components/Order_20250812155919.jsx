            ...
            <div key={order.id} className='col-md-6 col-lg-4'>
              ...
              >
                <div className='card-body'>
                  ...
                  <p className='mb-1'>
                    <strong>{t('date')}:</strong> {formatDate(order.date)}
                  </p>

                  <p className='mb-1'>
                    <strong>{t('status')}:</strong> {order.status || t('na')}
                  </p>

                  <h6 className='fw-bold mt-3' style={{ color: '#28a745' }}>
                    {t('items')}:
                  </h6>
                  ...
                  <ul className='list-unstyled'>
                    {order.items?.map((item, index) => (
                      ...
                      >
                        <span>{item.productName || t('unnamedProduct')}</span>
                        <span className='text-muted'>
                          {t('quantityPrefix', { qty: item.qty || 0 })}
                        </span>
                      </li>
                    ))}
                  </ul>
